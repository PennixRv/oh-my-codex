...
import { resolveTeamWorkerCliForResolvedLaunchArgs, sanitizeTeamName } from '../tmux-session.js';
...
async function initUnbornRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omx-runtime-unborn-repo-'));
  execFileSync('git', ['init'], { cwd, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd, stdio: 'ignore' });
  execFileSync('git', ['config', 'user.name', 'Test User'], { cwd, stdio: 'ignore' });
  return cwd;
...
describe('runtime', () => {
  ...
  it('resolveWorkerLaunchArgsFromEnv honors inherited leader model from the dedicated env path for non-exact roles', () => {
    const args = resolveWorkerLaunchArgsFromEnv(
      {
        OMX_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen',
        [TEAM_WORKER_INHERITED_MODEL_ENV]: 'gpt-4.1',
      },
      'executor',
      undefined,
      resolveAgentReasoningEffort('executor'),
    ...
    assert.deepEqual(args, ['--no-alt-screen', '-c', 'model_reasoning_effort="medium"', '--model', 'gpt-4.1']);
  });

  it('resolveWorkerLaunchArgsFromEnv keeps planner on exact gpt-5.4-mini when inherited leader is gpt-5.5', () => {
    withIsolatedDefaultModelEnv(() => {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMX_TEAM_WORKER_LAUNCH_ARGS: '--dangerously-bypass-approvals-and-sandbox --model gpt-5.5',
          [TEAM_WORKER_INHERITED_MODEL_ENV]: 'gpt-5.5',
        },
        'planner',
        undefined,
  ...
  it('resolveTeamWorkerCliForResolvedLaunchArgs derives codex from exact-role resolved launch args', () => {
    withIsolatedDefaultModelEnv(() => {
      ...
      const workerCli = resolveTeamWorkerCliForResolvedLaunchArgs(1, 1, resolvedLaunchArgs, {
  ...
  it('waitForWorkerStartupEvidence treats startup dispatch active-task confirmation as settled for Codex', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omx-runtime-codex-dispatch-active-'));
    try {
      await initTeamState('codex-dispatch-active', 'startup evidence test', 'executor', 1, cwd);
      const startupRequestId = 'startup-request-1';
      await writeAtomic(
        join(cwd, '.omx', 'state', 'team', 'codex-dispatch-active', 'dispatch', 'requests.json'),
        JSON.stringify([
          {
            request_id: startupRequestId,
            kind: 'inbox',
            team_name: 'codex-dispatch-active',
            to_worker: 'worker-1',
            worker_index: 1,
            pane_id: '%42',
            trigger_message: 'Read inbox',
      ...
      const evidence = await waitForWorkerStartupEvidence({
        teamName: 'codex-dispatch-active',
        workerName: 'worker-1',
        workerCli: 'codex',
        cwd,
        startupRequestId,
      ...
      assert.equal(evidence, 'dispatch_active_task');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
...
`,
        ...
        async () => {
          ...
          process.env.OMX_TEAM_SKIP_READY_WAIT = '0';
...
case "$1" in
  ...
  split-window)
    ...
    ;;
  capture-pane)
    printf 'startup still bootstrapping without prompt\\n'
    exit 0
    ;;
  send-keys|resize-pane|select-layout|set-window-option|select-pane|set-hook|run-shell|kill-pane|kill-session)
    exit 0
        ...
        async ({ tmuxLogPath }) => {
          ...
          const runtime = await withoutTeamWorkerEnv(() =>
            startTeam(
              ...
              'interactive startup should recover when startup evidence never arrives',
              'executor',
              1,
              [{ subject: 's', description: 'd', owner: 'worker-1' }],
              cwd,
...
`,
  ...
  it('startTeam falls back to single-workspace mode in an unborn git repo when no explicit worktree mode is requested', async () => {
    const cwd = await initUnbornRepo();
    const binDir = join(cwd, 'bin');
      ...
      runtime = await withPromptModeCodexEnv(binDir, {}, () =>
        withoutTeamWorkerEnv(() =>
          startTeam(
            ...
            'executor',
            1,
            [{ subject: 'create file', description: 'create result file', owner: 'worker-1' }],
            cwd,
          )),
      );

      assert.equal(runtime.config.workspace_mode, 'single');
      assert.deepEqual(runtime.config.worktree_mode, { enabled: false });
      assert.equal(runtime.config.workers[0]?.worktree_path, undefined);
      assert.equal(runtime.config.workers[0]?.worktree_repo_root, undefined);
    } finally {
      if (runtime) {
...
process.on('SIGTERM', () => process.exit(0));`,
  ...
  it('startTeam rejects duplicate active same-name team state without mutating existing files', async () => {
    ...
    try {
      ...
      await initTeamState(
        ...
        {
          ...process.env,
          OMX_SESSION_ID: 'sess-existing-team',
          OMX_TEAM_WORKER_LAUNCH_MODE: 'prompt',
        },
      );
      ...
      await writeWorkerIdentity('dup-team', 'worker-1', {
        name: 'worker-1',
        index: 1,
        role: 'executor',
        assigned_tasks: ['1'],
      ...
      const existingConfig = await readTeamConfig('dup-team', cwd);
      assert.ok(existingConfig);
      existingConfig.worker_launch_mode = 'prompt';
      if (existingConfig.workers[0]) {
        existingConfig.workers[0].pid = process.pid;
      }
      await saveTeamConfig(existingConfig, cwd);
      ...
      const fakeGeminiPath = join(binDir, 'gemini');
      await mkdir(binDir, { recursive: true });
      await writeFakePromptWorkerBinary(fakeGeminiPath, 'setTimeout(() => {}, 5000);\nprocess.on(\'SIGTERM\', () => process.exit(0));\n');
      ...
      await assert.rejects(
        ...
        /leader_session_conflict: active team exists \(dup-team\)|team_name_conflict: active team state already exists/,
      );
  ...
  it('startTeam archives a prior terminal same-name run before creating a clean replacement run', async () => {
    ...
    try {
      ...
      process.env.PATH = `${binDir}:${process.env.PATH ?? ''}`;
      process.env.OMX_SESSION_ID = 'sess-archive-terminal';
      process.env.OMX_TEAM_WORKER_LAUNCH_MODE = 'prompt';
      process.env.OMX_TEAM_WORKER_CLI = 'gemini';
      ...
      await createTask('archive-team', {
        subject: 'preserve me',
        description: 'terminal run should be archived before replacement',
        status: 'completed',
      }, cwd);
      await writeFile(
        join(cwd, '.omx', 'state', 'team', 'archive-team', 'phase.json'),
        JSON.stringify({ current_phase: 'complete', updated_at: new Date().toISOString() }, null, 2),
        'utf-8',
      );
      await mkdir(join(cwd, '.omx', 'team', 'archive-team'), { recursive: true });
      await writeFile(join(cwd, '.omx', 'team', 'archive-team', 'runtime.txt'), 'runtime artifact', 'utf-8');
      ...
      const stateArchiveRoot = join(cwd, '.omx', 'state', 'team-archive');
      const runtimeArchiveRoot = join(cwd, '.omx', 'team-archive');
      const archivedStateEntries = await readdir(stateArchiveRoot, { withFileTypes: true });
      const archivedRuntimeEntries = await readdir(runtimeArchiveRoot, { withFileTypes: true });
      const stateArchiveDir = archivedStateEntries.find((entry) => entry.isDirectory() && entry.name.startsWith('archive-team-') && entry.name.includes('-previous-terminal-run'));
      const runtimeArchiveDir = archivedRuntimeEntries.find((entry) => entry.isDirectory() && entry.name.startsWith('archive-team-') && entry.name.includes('-previous-terminal-run'));
      assert.ok(stateArchiveDir, 'expected prior state to be archived');
      assert.ok(runtimeArchiveDir, 'expected prior runtime artifacts to be archived');

      const archivedMeta = JSON.parse(
        await readFile(join(stateArchiveRoot, stateArchiveDir!.name, 'archived-meta.json'), 'utf-8'),
      ) as { reason?: string; archived_from?: string };
      assert.equal(archivedMeta.archived_from, 'archive-team');
      assert.equal(archivedMeta.reason, 'previous-terminal-run');

      const archivedTask = JSON.parse(
        await readFile(join(stateArchiveRoot, stateArchiveDir!.name, 'tasks', 'task-1.json'), 'utf-8'),
      ) as { subject?: string };
      assert.equal(archivedTask?.subject, 'preserve me');
      assert.equal(existsSync(join(runtimeArchiveRoot, runtimeArchiveDir!.name, 'runtime.txt')), true);

      const liveTask = await readTask(runtime.teamName, '1', cwd);
      assert.equal(liveTask?.subject, 'new subject');
      assert.equal(existsSync(join(cwd, '.omx', 'team', 'archive-team', 'runtime.txt')), false);
    } finally {
      if (runtime) {
        await shutdownTeam(runtime.teamName, cwd, { force: true }).catch(() => {});
      }
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          receiptNotifier = setInterval(() => {
            void markPendingInboxDispatchesNotified('team-interactive-cleanup', cwd, { toWorker: 'worker-1' }).catch(() => {});
          }, 20);
...
`,
        ...
        async () => {
          ...
          receiptNotifier = setInterval(() => {
            void markPendingInboxDispatchesNotified('team-pane-pid', cwd, { toWorker: 'worker-1' }).catch(() => {});
          }, 20);
  ...
  it('startTeam avoids duplicate startup replay and terminalizes startup requests when evidence never arrives', async () => {
...
`,
  ...
  it('startTeam sends codex startup trigger directly once ready prompt is observed', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omx-runtime-ready-prompt-direct-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omx-runtime-ready-prompt-direct-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
count_file="${cwd}/capture-count"
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const runtime = await withoutTeamWorkerEnv(() =>
            startTeam(
              'team-ready-prompt-direct',
              'ready prompt should use startup direct trigger for codex workers',
              'executor',
              1,
              [{ subject: 'w1', description: 'worker one', owner: 'worker-1' }],
              cwd,
          ...
          assert.equal(requests.length, 1);
          assert.equal(requests[0]?.intent, 'startup-trigger');
          assert.equal(requests[0]?.transport_preference, 'transport_direct');
          assert.equal(requests[0]?.status, 'notified');

          const request = await readDispatchRequest(runtime.teamName, requests[0]!.request_id, cwd);
          assert.equal(request?.transport_preference, 'transport_direct');

          const timingPath = join(cwd, '.omx', 'state', 'team', runtime.teamName, 'startup-timing.json');
          const timing = JSON.parse(await readFile(timingPath, 'utf-8')) as {
            events: Array<{ phase: string; reason?: string }>;
          };
          assert.ok(timing.events.some((event) => event.phase === 'ready_wait_end' && event.reason === undefined));
          assert.ok(timing.events.some((event) => event.phase === 'direct_fallback' && event.reason === 'startup_direct_trigger_sent:ready_prompt'));
          assert.ok(timing.events.some((event) => event.phase === 'startup_evidence' && event.reason === 'ready_prompt'));
          assert.equal(timing.events.some((event) => event.phase === 'hook_receipt'), false);
        },
      );
    } finally {
      if (runtimeTeamName) await shutdownTeam(runtimeTeamName, cwd, { force: true }).catch(() => {});
  ...
  it('startTeam marks ready-prompt timeout without startup evidence as recoverable and terminalizes the request', async () => {
...
`,
  ...
  it('startTeam accepts hook-confirmed active-task startup evidence after a ready shortcut timeout', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omx-runtime-ready-shortcut-active-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
    ...
    const previousStartupDispatchRetries = process.env.OMX_TEAM_STARTUP_DISPATCH_RETRIES;
    const previousStartupDispatchRetryDelay = process.env.OMX_TEAM_STARTUP_DISPATCH_RETRY_DELAY_MS;
    const teamName = `tsa-${process.pid}-${Date.now().toString(36)}`;
    let receiptNotifier: NodeJS.Timeout | null = null;
    let runtimeTeamName: string | null = null;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omx-runtime-ready-shortcut-active-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async () => {
          ...
          process.env.OMX_TEAM_WORKER_LAUNCH_MODE = 'interactive';
          process.env.OMX_TEAM_WORKER_CLI = 'codex';
          process.env.OMX_TEAM_READY_TIMEOUT_MS = '45000';
          process.env.OMX_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS = '500';
          process.env.OMX_TEAM_STARTUP_DISPATCH_RETRIES = '1';
          process.env.OMX_TEAM_STARTUP_DISPATCH_RETRY_DELAY_MS = '50';

          receiptNotifier = setInterval(async () => {
            await markPendingInboxDispatchesNotified(teamName, cwd, {
              toWorker: 'worker-1',
              lastReason: 'tmux_send_keys_confirmed_active_task',
            }).catch(() => {});
          }, 20);
          ...
          const runtime = await withoutTeamWorkerEnv(() =>
            startTeam(
              teamName,
              'hook-confirmed active task should settle startup',
              'executor',
              1,
              [{ subject: 'w1', description: 'worker one', owner: 'worker-1' }],
              cwd,
          ...
          assert.equal(request?.last_reason, 'tmux_send_keys_confirmed_active_task');

          const timingPath = join(cwd, '.omx', 'state', 'team', runtime.teamName, 'startup-timing.json');
          const timing = JSON.parse(await readFile(timingPath, 'utf-8')) as {
            events: Array<{ phase: string; reason?: string; ok?: boolean }>;
          };
          assert.ok(
            timing.events.some((event) => event.phase === 'ready_wait_end' && event.reason === 'shortcut_timeout' && event.ok === false),
          );
          assert.ok(
            timing.events.some((event) => event.phase === 'startup_evidence' && event.reason === 'dispatch_active_task' && event.ok === true),
          );
        },
      );
    } finally {
...
`),
        ...
        async () => {
          ...
          runtime = await withoutTeamWorkerEnv(() =>
            startTeam(
              teamName,
              'interactive startup should recover from no-evidence startup even while panes stay alive',
              'executor',
              2,
              [
          ...
          for (const request of requests) {
            assert.equal(request.intent, 'startup-trigger');
            const persisted = await readDispatchRequest(runtime.teamName, request.request_id, cwd);
            assert.notEqual(persisted?.status, 'pending');
          }
...
`),
        ...
        async () => {
          let runtimeTeamName = 'team-materialize-before-evidence';
          delete process.env.TMUX;
          process.env.TMUX_PANE = '%1';
          process.env.OMX_TEAM_WORKER_LAUNCH_MODE = 'interactive';
          process.env.OMX_TEAM_WORKER_CLI = 'codex';
          ...
          receiptFailer = setInterval(() => {
            void (async () => {
              const teamEntries = await readdir(join(cwd, '.omx', 'state', 'team'), { withFileTypes: true }).catch(() => []);
              const activeTeamName = teamEntries.find((entry) => entry.isDirectory())?.name
                ?? await resolveRuntimeTeamName(cwd, 'team-materialize-before-evidence');
          ...
          let lastObservedTeamName = runtimeTeamName;
          for (let attempt = 0; attempt < 400; attempt += 1) {
            const teamEntries = await readdir(join(cwd, '.omx', 'state', 'team'), { withFileTypes: true }).catch(() => []);
            runtimeTeamName = teamEntries.find((entry) => entry.isDirectory())?.name
              ?? await resolveRuntimeTeamName(cwd, 'team-materialize-before-evidence');
            lastObservedTeamName = runtimeTeamName;
          ...
          assert.equal(
            ...
            true,
            `worker-2 durable state should exist before worker-1 startup evidence handling settles (last observed team=${lastObservedTeamName})`,
          );

          const outcome = await observedTeamPromise;
          assert.equal(outcome.ok, true);
          const runtime = (outcome as { ok: true; runtime: TeamRuntime }).runtime;
  ...
  list-panes)
    case "$*" in
      ...
      *"-t %14 -F #{pane_pid}"*)
        ...
        ;;
      *"-t leader:0 "*)
        printf "%%11\\tzsh\\tzsh\\n%%12\\tnode\\tnode /tmp/bin/omx.js hud --watch OMX_TMUX_HUD_LEADER_PANE=%%11 OMX_SESSION_ID=team-shutdown-pane-reconcile-session\\n%%13\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-pane-reconcile/runtime/worker-1-startup.sh'\\n%%14\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-pane-reconcile/runtime/worker-2-startup.sh'\\n%%15\\tcodex\\tcodex unrelated-not-worker\\n%%16\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-pane-reconcile/runtime/worker-3-startup.sh'\\n%%17\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-pane-reconcile/runtime/worker-4-startup.sh'\\n%%18\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-pane-reconcile/runtime/worker-5-startup.sh'\\n%%19\\tpowershell.exe\\tpowershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -EncodedCommand ${powershellWorkerCommand}\\n%%20\\tzsh\\techo 'OMX_TEAM_INTERNAL_WORKER=team-shutdown-pane-reconcile/worker-7'\\n%%21\\tzsh\\tprintf 'OMX_TEAM_INTERNAL_WORKER=team-shutdown-pane-reconcile/worker-8 codex'\\n%%22\\tzsh\\tenv OMX_TEAM_INTERNAL_WORKER=team-shutdown-pane-reconcile/worker-9 bash -lc 'echo codex'\\n"
        if [ -f "$restored_marker" ]; then
          printf "%%44\\tnode\\tnode /tmp/bin/omx.js hud --watch OMX_TMUX_HUD_LEADER_PANE=%%11 OMX_SESSION_ID=team-shutdown-pane-reconcile-session\\n"
        fi
        exit 0
        ;;
      *)
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          config.tmux_pane_owner_id = 'team:team-shutdown-pane-reconcile';
  ...
  list-panes)
    case "$*" in
      *"-F #{pane_dead} #{pane_pid}"*)
        ...
        ;;
      *"-t leader:0 "*)
        printf "%%11\\tpwsh\\tpwsh\\n%%12\\tnode\\tnode /tmp/bin/omx.js hud --watch OMX_TMUX_HUD_LEADER_PANE=%%11 OMX_SESSION_ID=team-shutdown-win32-split-session\\n%%13\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-win32-split/runtime/worker-1-startup.sh'\\n%%14\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-win32-split/runtime/worker-2-startup.sh'\\n"
        exit 0
        ;;
      *)
        exit 1
  ...
  show-option)
    case "$*" in
      ...
      *"-p -t %12 @omx_team_pane_owner_id"*)
        echo "team:team-shutdown-win32-split"
        ;;
      *"-p -t %13 @omx_team_pane_owner_id"|*"-p -t %14 @omx_team_pane_owner_id"*)
        echo "team:team-shutdown-win32-split"
        ;;
      *)
        exit 1
  ...
  list-panes)
    case "$*" in
      *"-F #{pane_dead} #{pane_pid}"*)
        ...
        ;;
      *"-t leader:0 "*)
        printf "%%11\\tpwsh\\tpwsh\\n%%22\\tnode\\tnode /tmp/bin/omx.js hud --watch OMX_TMUX_HUD_LEADER_PANE=%%99 OMX_SESSION_ID=unrelated-session\\n%%23\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-win32-stale-topo/runtime/worker-1-startup.sh'\\n%%24\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-win32-stale-topo/runtime/worker-2-startup.sh'\\n"
        exit 0
        ;;
      *)
        exit 1
  ...
  show-option)
    case "$*" in
      *"-p -t %11 @omx_team_pane_owner_id"*)
        ...
        ;;
      *"-p -t %23 @omx_team_pane_owner_id"|*"-p -t %24 @omx_team_pane_owner_id"*)
        echo "team:team-win32-stale-topo"
        ;;
      *)
        exit 1
...
`,
          ...
          async ({ tmuxLogPath }) => {
            ...
            config.leader_pane_id = '%11';
            config.hud_pane_id = '%12';
            config.tmux_pane_owner_id = 'team:team-win32-stale-topo';
  ...
  list-panes)
    case "$*" in
      *"-F #{pane_dead} #{pane_pid}"*)
        ...
        ;;
      *"-t leader:0 "*)
        printf "%%11\\tzsh\\tzsh\\n%%12\\tnode\\tnode /tmp/bin/omx.js hud --watch OMX_TMUX_HUD_LEADER_PANE=%%11 OMX_SESSION_ID=team-shutdown-shared-session-owner\\n%%13\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-shared-session/runtime/worker-1-startup.sh'\\n%%14\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-shared-session/runtime/worker-2-startup.sh'\\n"
        exit 0
        ;;
      *)
        exit 1
...
case "$1" in
  ...
  list-panes)
    case "$*" in
      *"-t leader:0 "*)
        printf "%%11\\tzsh\\tzsh\\n%%12\\tnode\\tnode /tmp/bin/omx.js hud --watch OMX_TMUX_HUD_LEADER_PANE=%%11 OMX_SESSION_ID=team-shutdown-restore-hud-session\\n%%13\\tcodex\\texec /bin/sh '/tmp/.omx/state/team/team-shutdown-restore-hud/runtime/worker-1-startup.sh'\\n"
        exit 0
        ;;
      *)
        exit 1
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          config.leader_pane_id = '%11';
          config.hud_pane_id = '%12';
          config.tmux_pane_owner_id = 'team:team-shutdown-restore-hud';
          config.workers[0]!.pane_id = '%13';
    ...
    try {
      await withTempTmuxSession(async (fixture) => {
        ...
        const runtimeDir = join(
          ...
          'state',
          'team',
          'team-shutdown-socket-identity',
          'runtime',
        );
        await mkdir(runtimeDir, { recursive: true });
        const workerStartupScript = join(runtimeDir, 'worker-1-startup.sh');
        await writeFile(workerStartupScript, '#!/bin/sh\nsleep 300\n', { encoding: 'utf-8' });
        await chmod(workerStartupScript, 0o755);
        ...
        const workerPaneId = execFileSync(
          ...
          [
            ...
            '#{pane_id}',
            '-t',
            fixture.windowTarget,
            `exec /bin/sh '${workerStartupScript}'`,
          ],
        ...
        config.tmux_pane_owner_id = 'team:team-shutdown-socket-identity';
...
`,
  ...
  it('assignTask injects approved handoff context when the persisted approved binding remains ready', async () => {
    ...
    try {
      ...
      await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
      const configPath = teamStateTestPath(cwd, 'team', 'team-approved-followup', 'config.json');
      ...
      await writeWorkerIdentity('team-approved-followup', 'worker-1', {
        name: 'worker-1',
        index: 1,
        role: 'executor',
        assigned_tasks: ['1'],
