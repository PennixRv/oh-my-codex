...
describe('omx session search', () => {
  ...
  it('searches generated project runtime Codex homes in a project repo', async () => {
    ...
    try {
      ...
      assert.deepEqual(parsed.results.map((entry) => entry.session_id).sort(), ['default-session', 'runtime-session']);
  ...
  it('searches associated madmax boxed run roots without leaking raw run paths', async () => {
    ...
    try {
      ...
      assert.equal(parsed.results.some((entry) => entry.transcript_path.includes(runsRoot)), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
