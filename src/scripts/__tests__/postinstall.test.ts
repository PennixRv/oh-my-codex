...
describe("runPostinstall", () => {
  it("logs a manual setup reminder and preserves prior setup completion state for bumped global installs", async () => {
    ...
    try {
      ...
      assert.equal(result.status, "reminder-logged");
      assert.match(logs.join("\n"), /Run `omx setup` when you're ready/i);
      assert.match(logs.join("\n"), /use `omx update` to refresh npm plus setup together/i);
