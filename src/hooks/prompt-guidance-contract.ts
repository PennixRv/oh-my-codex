...
const ROOT_TEMPLATE_PATTERNS = [
  rx('persistent bootstrap contract'),
  ...
  rx('OMX runtime manipulation.*agent responsibilit(y|ies)'),
  rx('Keep going unless blocked'),
  rx('local overrides?.*non-conflicting instructions'),
  rx('smallest useful tool loop|reflexive web/tool churn'),
  ...
  rx('Route to `researcher`'),
  rx('technology is already chosen'),
  rx('Route to `dependency-expert`'),
  rx('mixed routing deliberately'),
  rx('keyword registry'),
  rx('analyze.*investigate.*\\$analyze'),
  rx('Stop when the task is verified complete'),
  ...
  rx('validation.*cannot run|validation gaps? are explicit'),
];
