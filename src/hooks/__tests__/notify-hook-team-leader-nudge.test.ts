...
describe('notify-hook team leader nudge shutdown suppression', () => {
...
`);
      ...
      assert.equal(existsSync(join(teamDir, 'leader-attention.json')), false);
  ...
  it('does not persist bookkeeping when team is removed after the final liveness check', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.equal(existsSync(teamDir), false);
      assert.equal(existsSync(join(teamDir, 'leader-attention.json')), false);
