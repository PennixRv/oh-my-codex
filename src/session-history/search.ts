...
interface SessionSearchCodexHomeSource {
  dir: string;
  publicLabel: string;
}
...
export async function resolveSessionSearchCodexHomeDirs(
  options: Pick<SessionSearchOptions, 'cwd' | 'codexHomeDir' | 'codexHomeDirs'> = {},
): Promise<string[]> {
...
async function resolveSessionSearchCodexHomeSources(options: SessionSearchOptions): Promise<SessionSearchCodexHomeSource[]> {
  if (options.codexHomeDirs && options.codexHomeDirs.length > 0) {
    const dirs = await normalizeExistingCodexHomeDirs(options.codexHomeDirs);
    return dirs.map((dir) => ({ dir, publicLabel: dir }));
  }
  if (options.codexHomeDir) {
    const dirs = await normalizeExistingCodexHomeDirs([options.codexHomeDir]);
    return dirs.map((dir) => ({ dir, publicLabel: dir }));
  }

  const cwd = options.cwd ?? process.cwd();
  const defaultDir = codexHome();
  const projectHomes = await discoverProjectRuntimeCodexHomes(cwd);
  const dirs = await normalizeExistingCodexHomeDirs([defaultDir, ...projectHomes.map((home) => home.path)]);
  const projectHomeByRealpath = new Map<string, SessionSearchCodexHomeSource>();
  for (const home of projectHomes) {
    const absolute = resolve(home.path);
    const key = await realpath(absolute).catch(() => absolute);
    projectHomeByRealpath.set(key, {
      dir: absolute,
      publicLabel: home.publicLabel ?? absolute,
    });
  }
  return dirs.map((dir) => {
    const absolute = resolve(dir);
    const key = projectHomeByRealpath.has(absolute)
      ? absolute
      : absolute;
    return projectHomeByRealpath.get(key) ?? { dir: absolute, publicLabel: absolute };
  });
}
...
export async function searchSessionHistory(options: SessionSearchOptions): Promise<SessionSearchReport> {
  ...
  for (const codexHomeSource of codexHomeSources) {
    ...
    for (const result of sourceSearch.results) results.push(result);
  }
