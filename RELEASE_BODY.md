# oh-my-codex-pennix v0.18.61

> Release notes template for the Pennix fork. The tag workflow regenerates this file into the final GitHub release body.

## Summary

This release republishes the first bounded upstream integration wave on top of the Pennix fork with the release pipeline repaired. It keeps the wave-1 setup/config/native-hook/worker-bootstrap convergence changes, and adds a release-hardening fix so explicit setup stamp tests derive expectations from the active package version instead of a stale pinned version.
...
- Release-gating explicit setup stamp tests now read the active package version at runtime, so future patch bumps do not break GitHub CI or tag-triggered release publication with stale hard-coded expectations.
...
- No merged PRs. `0.18.61` is a direct release-line commit on the Pennix fork.
...
**Full Changelog**: [`v0.18.59...v0.18.61`](https://github.com/PennixRv/oh-my-codex/compare/v0.18.59...v0.18.61)
