...
The official plugin manifest must continue to omit `agents` and `prompts`; lifecycle hooks are delivered through the plugin hook bundle via `hooks = ./hooks/hooks.json`. Legacy setup mode installs prompts/native agents and `.codex/hooks.json`; plugin setup mode removes archived legacy prompt copies, refreshes setup-owned native agent TOMLs for `agent_type` routing, removes only stale generated/non-installable native-agent files, and removes setup-managed `.codex/hooks.json` wrappers instead of refreshing them.
