...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- Prefer setting `agent_type` to the narrowest installed OMX role for each native subagent dispatch; if Codex full-history fork mode inherits agent settings from the parent, follow that constraint instead of redundantly forcing `agent_type`, `model`, or `reasoning_effort`.',
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_policy: 'Parallel Codex subagents are allowed for independent work when each dispatch uses the narrowest compatible OMX role and respects Codex full-history fork inheritance constraints; phase completion must wait for active native subagent threads to finish.',
