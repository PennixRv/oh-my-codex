...
`;
  ...
  it('existing all-workers-idle hook still fires alongside per-worker', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Review unread worker messages first, then either continue the mainline, assign the next concrete task, reconcile results, or shut the team down if the team is complete\./, 'all-workers-idle nudge should prefer boundary-driven reconciliation over immediate polling');
      assert.match(tmuxLog, /Use `omx team status both-hooks` only when you need a fresh snapshot\./, 'all-workers-idle nudge should keep team status as an explicit snapshot tool');
    });
  });
});
